...
BASECPPFLAGS=	
CONFIGURE_CFLAGS=	-fomit-frame-pointer -funroll-loops  -ffast-math -Ofast -fno-signed-zeros -ffp-contract=fast -mmmx -msse -flto -maes -march=native  
...
SHLIB_SUFFIX=	.so
EXT_SUFFIX=	.cpython-36dm-darwin.so
...
BUILD_GNU_TYPE=	x86_64-apple-darwin17.0.0
HOST_GNU_TYPE=	x86_64-apple-darwin17.0.0
...
build_all_generate_profile:
	$(MAKE) build_all CFLAGS_NODIST="$(CFLAGS) $(PGO_PROF_GEN_FLAG) " LDFLAGS="$(LDFLAGS) $(PGO_PROF_GEN_FLAG) " LIBS="$(LIBS)"
...
build_all_use_profile:
	$(MAKE) build_all CFLAGS_NODIST="$(CFLAGS) $(PGO_PROF_USE_FLAG) " LDFLAGS="$(LDFLAGS) "
